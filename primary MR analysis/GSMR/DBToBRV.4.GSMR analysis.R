...
for (pheno2 in pheno2_all){
  for (tissue in tissue_all){
    file_name1 = paste('data/gene_expression/GTEx/mr_format/GSMR/SNP_in_outcome_clumped_txt/',tissue,'.ukb_roi_volume_may12_2019_phase1and2_pheno100_allchr_withA2.clumped.txt',sep='')
    ...
    write.table(result, paste('results/GSMR/DisorderToIDP/',pheno2,'.',tissue,'.GSMR.txt',sep=''), row.names = F, col.names= T, sep="\t", quote = F)
  }
}
