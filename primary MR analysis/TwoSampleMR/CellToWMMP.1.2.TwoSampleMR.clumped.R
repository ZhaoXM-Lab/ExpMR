...
for (outcome in outcome_all){
  ...
  for (tissue in tissue_all){
    ...
    outfilename = paste('data/gene_expression/Malhotra_lab/mr_format/TwoSampleMR/SNP_in_outcome/',tissue,'.',outcome,'.SNP_interc.txt',sep='')
...
for (outcome in outcome_all){
  for (tissue in tissue_all){
    command = paste('/public/home/yanganyi/Desktop/Software/plink1/plink ',
                    ...
                    ' --clump-r2 0.1',
                    ' --clump-kb 250',
                    ' --clump /public/home/yanganyi/Desktop/genexpMR/data/gene_expression/Malhotra_lab/mr_format/TwoSampleMR/SNP_in_outcome/',tissue,'.',outcome,'.SNP_interc.txt',
                    ' --clump-snp-field SNP',
                    ' --clump-field pval',
                    ' --out /public/home/yanganyi/Desktop/genexpMR/data/gene_expression/Malhotra_lab/mr_format/TwoSampleMR/SNP_in_outcome_clumped/',tissue,'.',outcome,
                    sep='')
...
for (outcome in outcome_all){
  for (tissue in tissue_all){
    ...
    clumped = fread(paste('SNP_in_outcome_clumped/', tissue,'.',outcome, '.clumped',sep=''), data.table = F) #原来的文件样子
