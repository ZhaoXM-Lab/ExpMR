...
for (outcome in outcome_all){
  ...
  for (tissue in tissue_all){
    filename = paste('data/gwas_summary/mr_format/SMR/brain_disorder/', tissue,'.EUR.signif_pairs_5e-8.txt',sep='')
...
#####################################
rm(list =ls())
setwd('/public/home/yanganyi/Desktop/genexpMR/data/gene_expression/GTEx/mr_format/TwoSampleMR/')
library(data.table)
library(tidyverse)
