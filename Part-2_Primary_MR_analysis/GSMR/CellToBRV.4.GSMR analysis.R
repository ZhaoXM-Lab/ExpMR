...
indiv = function(j){
  library("gsmr")
  source('codes/GSMR/gsmr_analysis.R')
...
pheno2_all = list.files('/share/home1/yanganyi/Desktop/Data/gwas_sumstat/yyc_postgwas/ukb/BrainVolume/')
pheno2_all = gsub('.txt', '', pheno2_all)


for (pheno2 in pheno2_all){
  for (tissue in tissue_all){
    file_name1 = paste('data/gene_expression/Malhotra_lab/mr_format/GSMR/',tissue,'.signif_pairs_5e-8.txt',sep='')
    ...
    write.table(result.all, paste('results/GSMR/CellToIDP/CellToVolume/',pheno2,'.',tissue,'.GSMR.txt',sep=''), row.names = F, col.names= T, sep="\t", quote = F)
  }
}