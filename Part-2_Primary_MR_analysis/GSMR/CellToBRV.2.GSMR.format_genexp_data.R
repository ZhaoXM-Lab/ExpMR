...
lookup_table = subset(lookup_table, lookup_table$SNP %in% g1000_freq$SNP)   #保留在g1000的，不然后面esi会有问题
...
for (i in 1:length(cell_types)) {
  ...
  names(data_mr) = c('SNP','gene','a1','a2','a1_freq','bzx','bzx_se','bzx_pval')
  
  data_mr = separate(data_mr, col = SNP, into = c("SNP1", "SNP2"),sep = ",")
  data_mr = data_mr[, c('SNP1','gene','a1','a2','a1_freq','bzx','bzx_se','bzx_pval')]
  names(data_mr)[1] = 'SNP'
  ...
  outname = paste('Malhotra_lab/mr_format/GSMR/', cell_type,'.signif_pairs_5e-8.txt',sep='')
