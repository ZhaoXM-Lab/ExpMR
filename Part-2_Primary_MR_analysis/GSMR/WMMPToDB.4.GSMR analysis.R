...
indiv = function(j, pheno2, tissue){
  library("gsmr")
...
#main--------------------------------------------------------------------------------
tissue_all = list.files('data/gwas_summary/mr_format/GSMR/IDP/')
...
for (pheno2 in pheno2_all){
  for (tissue in tissue_all){
    file_name1 = paste('data/gene_expression/GTEx/mr_format/GSMR/SNP_in_outcome_clumped_txt/',tissue,'.', pheno2,'.clumped.txt', sep='')
    ...
    file_name2 = paste('data/gwas_summary/mr_format/GSMR/brain_disorder/', pheno2,'.format.txt',sep='')
    ...
    result = indiv(1, pheno2, tissue)
    
    cat(tissue,' and ', pheno2, 'is OK.\n')
    write.table(result, paste('results/GSMR/IDPToDisorder/',pheno2,'.',tissue,'.GSMR.txt',sep=''), row.names = F, col.names= T, sep="\t", quote = F)
  }
}