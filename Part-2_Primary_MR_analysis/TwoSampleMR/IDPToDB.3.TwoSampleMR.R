...
run_tissue = function(tissue, outcome_data, outcome){
  library(TwoSampleMR)
  filename = paste('data/gene_expression/GTEx/mr_format/TwoSampleMR/SNP_in_outcome_clumped_txt/', 
                   tissue,'.', outcome, '.clumped.txt',sep='')
  ...
  if (!(file.exists(filename))){
    ...
    if (nrow(result) == 0) return(0)
      ...
      outfilename = paste('results/TwoSampleMR/IDPToDisorder/',outcome,'.',tissue,'.TwoSampleMR.txt',sep='')
