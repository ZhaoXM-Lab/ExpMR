...
for (outcome in outcome_all){
  filename = paste('data/gwas_summary/mr_format/PMR-Egger/brain_disorder/',outcome,'.format.txt', sep = '')
  file1 =  fread(filename, data.table = F) 
  
  for (tissue in tissue_all){
    filename = paste('data/gwas_summary/mr_format/PMR-Egger/IDP/', tissue,'.EUR.signif_pairs_5e-8.txt',sep='')
...
for (outcome in outcome_all){  
  for (tissue in tissue_all){
    if (!(file.exists(paste('data/gene_expression/GTEx/mr_format/PMR-Egger/SNP_in_outcome/', tissue,'.',outcome,'.SNP_interc.txt',sep='')))) next 
...
for (outcome in outcome_all){
  for (tissue in tissue_all){
    if (!(file.exists(paste('data/gene_expression/GTEx/mr_format/PMR-Egger/SNP_in_outcome/', tissue,'.',outcome,'.SNP_interc.txt',sep='')))) next
    file = fread(paste('data/gene_expression/GTEx/mr_format/PMR-Egger/SNP_in_outcome/', tissue,'.',outcome,'.SNP_interc.txt',sep=''), data.table = F)
    
    if (!(file.exists(paste('data/gene_expression/GTEx/mr_format/PMR-Egger/SNP_in_outcome_clumped/', tissue,'.',outcome, '.clumped',sep='')))) next
    clumped = fread(paste('data/gene_expression/GTEx/mr_format/PMR-Egger/SNP_in_outcome_clumped/', tissue,'.',outcome, '.clumped',sep=''), data.table = F) #原来的文件样子
    
    file  = subset(file, file$SNP %in% clumped$SNP)
    
    file = file[,-which(names(file)=='pval')]
    
    outname = paste('data/gene_expression/GTEx/mr_format/PMR-Egger/SNP_in_outcome_clumped_txt/', tissue,'.',outcome,'.clumped.txt',sep='')
