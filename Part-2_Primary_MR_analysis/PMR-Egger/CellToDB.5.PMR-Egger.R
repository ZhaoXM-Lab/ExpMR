...
for (pheno2 in pheno2_all){
  for (tissue in tissue_all){
    ...
    file_name1 = paste('data/gene_expression/Malhotra_lab/mr_format/PMR-Egger/SNP_in_outcome_clumped_txt/',tissue,'.clumped.txt',sep='')
    ...
    write.table(result, paste('results/PMR-Egger/CellToDisorder/',pheno2,'.',tissue,'.PMR-Egger.txt',sep=''), row.names = F, col.names= T, sep="\t", quote = F)
