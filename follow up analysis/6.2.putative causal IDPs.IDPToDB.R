...
#PMR-Egger--
file11 = fread('results/PMR-Egger/PMR-Egger.IDPToDisorder.all.txt', data.table = F)
file12 = fread('results/PMR-Egger/PMR-Egger.WMToDisorder.all.txt', data.table = F)
...
file1 = merge(file1, pheno_info, by.x = 'tissue', by.y = 'file_name', all.x = T)
file1 = file1[,c('pheno','pheno_name','b.pmregger','p.pmregger','p_pleiotropy.pmregger')]
names(file1)[2] = 'tissue'

#GSMR--
file21 = fread('results/GSMR/GSMR.IDPToDisorder.all.txt', data.table = F)
file22 = fread('results/GSMR/GSMR.WMToDisorder.all.txt', data.table = F)
file2 = rbind(file21, file22)
file2 = file2[,c('outcome','tissue','bxy','bxy_pval')]
names(file2) = c('pheno','tissue','b.gsmr','p.gsmr')
file2 = merge(file2, pheno_info, by.x = 'tissue', by.y = 'file_name', all.x = T)
file2 = file2[,c('pheno','pheno_name','b.gsmr','p.gsmr')]
names(file2)[2] = 'tissue'

#TwoSampleMR
file31 = fread('results/TwoSampleMR/TwoSampleMR.IDPToDisorder.all.txt', data.table = F)
file32 = fread('results/TwoSampleMR/TwoSampleMR.WMToDisorder.all.txt', data.table = F)
...
file3 = merge(file3, pheno_info, by.x = 'tissue', by.y = 'file_name', all.x = T)
file3 = file3[,c('pheno','pheno_name','method','b.wr_ivw','p.wr_ivw','Q_pval','pval_egger_intercept')]
names(file3)[2] = 'tissue'
...
summary = subset(summary, summary$pheno %in% DB_selec)
...
signif_gene$IDPToDisorder = paste(signif_gene$tissue, signif_gene$pheno, sep = '-')

write.table(signif_gene, paste('results/','signif_IDPToDisorder.fdr.new.txt',sep=''), sep="\t", row.names=F, col.names=T, quote=F)