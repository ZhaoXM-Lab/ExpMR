...
pheno2_all = list.files('/public/home/yanganyi/Desktop/Data/gwas_sumstat/yyc_postgwas/ukb/BrainVolume/')
pheno2_all = gsub('.txt', '', pheno2_all)


for (pheno2 in pheno2_all){
  for (tissue in tissue_all){

    file_name1 = paste('data/gene_expression/GTEx/mr_format/PMR-Egger/SNP_in_outcome_clumped_txt/',tissue,'.ukb_roi_volume_may12_2019_phase1and2_pheno100_allchr_withA2.clumped.txt',sep='')
    file10 = fread(file_name1, data.table = F)
    
    xmat0 = fread(paste('data/gwas_summary/mr_format/PMR-Egger/brain_disorder/xmat/',tissue,'.xmat.gz',sep=''), data.table=F, header=T)
    xmat0 = xmat0[-1, ]
    ...
    n1 = trait_info[which(trait_info$Trait == tissue), 'N_EUR']
    n2 = 19629 
    
    
    result = run_mr(mr_file10, mr_file20, xmat0, n1, n2, 1)
    
    write.table(result, paste('results/PMR-Egger/DisorderToIDP/',pheno2,'.',tissue,'.PMR-Egger.txt',sep=''), row.names = F, col.names= T, sep="\t", quote = F)
