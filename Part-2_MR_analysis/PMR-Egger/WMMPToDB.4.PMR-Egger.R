...
pheno2_all = trait_info[which(trait_info$Category == 'GWAS'), 'Trait']


for (pheno2 in pheno2_all){
  for (tissue in tissue_all[c(4)]){

    file_name1 = paste('data/gene_expression/GTEx/mr_format/PMR-Egger/SNP_in_outcome_clumped_txt/',tissue,'.',pheno2,'.clumped.txt',sep='')
    if (!(file.exists(file_name1)))  next
    file10 = fread(file_name1, data.table = F)
    
    
    if (!(file.exists(paste('data/gwas_summary/mr_format/PMR-Egger/IDP/xmat/',tissue,'.xmat.gz',sep=''))))  next
    xmat0 = fread(paste('data/gwas_summary/mr_format/PMR-Egger/IDP/xmat/',tissue,'.xmat.gz',sep=''), data.table=F, header=T)
    xmat0 = xmat0[-1, ]
    xmat0 = xmat0[, -which(names(xmat0) %in% c('FID','IID'))]
    if (is.vector(xmat0)) next
    xmat0 = apply(xmat0, 2, as.numeric)
    

    file_name2 = paste('data/gwas_summary/mr_format/PMR-Egger/brain_disorder/', pheno2,'.format.txt',sep='')
    file20 = fread(file_name2, data.table = F)

    file20 = subset(file20, !(is.na(file20$Zscore)))
    file20[which(file20$Zscore == Inf), 'Zscore'] = 10
    file20[which(file20$Zscore == -Inf), 'Zscore'] = -10
    
    
    snp_all = Reduce(intersect, list(colnames(xmat0), file10$SNP, file20$SNP))
    if (length(snp_all) <= 1) next
    ...
    n1 = 35741
    n2 = trait_info[which(trait_info$Trait == pheno2), 'N_EUR']
    
    
    result = run_mr(mr_file10, mr_file20, xmat0, n1, n2, 1)
    
    write.table(result, paste('results/PMR-Egger/IDPToDisorder/',pheno2,'.',tissue,'.PMR-Egger.txt',sep=''), row.names = F, col.names= T, sep="\t", quote = F)
