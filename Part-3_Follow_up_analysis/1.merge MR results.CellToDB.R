...
write.table(merge.data, paste('results/TWAS/','TWAS.14BrainTissuesToDisorder.all.txt',sep = ''), 
...
file = fread('results/TWAS/TWAS.14BrainTissuesToDisorder.all.txt', data.table = F)
...
stopCluster(cl)
write.table(result, 'results/TWAS/TWAS.14BrainTissuesToDisorder.allmosetsignif.txt', sep="\t", row.names=F, col.names=T, quote=F)
...
for (pheno in pheno_all){
  for (tissue in tissue_all){
    file_name = paste('results/PMR-Egger/CellToDisorder/', pheno,'.',tissue, '.PMR-Egger.txt', sep='')
    new.data = read.table(file_name, header = T, sep = "\t", stringsAsFactors = F)
...
write.table(merge.data, "results/PMR-Egger/PMR-Egger.CellToDisorder.all.txt", sep="\t", row.names=F, col.names=T, quote=F)
...
for (i in 1:length(pheno_all)){
  for (j in 1:length(tissue_all)){
    pheno = pheno_all[i]
    tissue = tissue_all[j]
    file_name = paste('results/SMR/CellToDisorder/', pheno,'.',tissue,'.smr', sep='')
...
write.table(merge.data, "results/SMR/SMR.CellToDisorder.all.txt", sep="\t", row.names=F, col.names=T, quote=F)
...
for (i in 1:length(pheno_all)){
  for (j in 1:length(tissue_all)){
    pheno = pheno_all[i]
    tissue = tissue_all[j]
    file_name = paste('results/TwoSampleMR/CellToDisorder/', pheno,'.',tissue,'.TwoSampleMR.txt', sep='')
    new.data = read.table(file_name, header = T, sep = "\t", stringsAsFactors = F)
...
write.table(merge.data, "results/TwoSampleMR/TwoSampleMR.CellToDisorder.all.txt", sep="\t", row.names=F, col.names=T, quote=F)
...
for (pheno in pheno_all){
  for (tissue in tissue_all){
    file_name = paste('results/GSMR/CellToDisorder/', pheno,'.',tissue, '.GSMR.txt', sep='')
    new.data = read.table(file_name, header = T, sep = "\t", stringsAsFactors = F)
...
write.table(merge.data, paste('results/GSMR/GSMR.CellToDisorder.all.txt',sep=''), sep="\t", row.names=F, col.names=T, quote=F)
