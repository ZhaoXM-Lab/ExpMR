...
f1$trait_type = 'IDP'
f1 = f1[!grepl('FA', f1$pheno), ]  #Brain regional volume
...
p <- ggplot(plot) +
  ...
  labs(x = NULL, y = NULL) +  # 移除坐标轴标题
  ggtitle("Shared genes in specific cell types between brain regional volume and psychiatric disorders") + 
  coord_flip()

for (i in c(4:ncol(plot))){
  if (!(names(plot)[i] %in% PD_all)) {
    p <- p + 
      eval(parse(text = paste("geom_point( aes(x=group, y= plot[,",i,"]),shape = 21,color='black',fill='#4AB74C', size=4)"))) 
  }
  if (names(plot)[i] %in% PD_all){
    p <- p + 
...
plot = plot[,names(plot) %in% c(names(table_IDP), names(table_ND))] #只保留IDP和BCP的列
plot = rbind(plot, Total = colSums(plot))   #行和
plot = plot[,-which(plot['Total',]==0)]
plot = plot[, c(-1,-2)]
plot = plot[-nrow(plot), ]
...
p <- ggplot(plot) +
  ...
  ylab("Traits") +
  scale_y_continuous(limits=c(1,ncol(plot)-3), breaks=seq(1,ncol(plot)-3,1),labels = c(names(plot)[4:ncol(plot)])) +
  theme(axis.text.x=element_text(angle=90, size=12, vjust = 0.25, hjust = 1,color = 'black'),
        axis.text.y=element_text(size=12, color = 'black'), 
        plot.title = element_text(size=15, color = 'black',face="bold")) +
  labs(x = NULL, y = NULL) +
  ggtitle("Shared genes in specific cell types between brain regional volume and neurological disorders") + 
  coord_flip()
...
for (i in c(4:ncol(plot))){
  ...
  if (names(plot)[i] %in% ND_all){
    p <- p + 
      eval(parse(text = paste("geom_point( aes(x=group, y= plot[,",i,"]),shape = 21,color='black',fill='#E46C2F', size=4)")))
  } 
}
p
...
p <- ggplot(plot) +
  ...
  labs(x = NULL, y = NULL) +
  ggtitle("Shared cell type-specific genes between brain regional volume and behavioral-cognitive phenotypes") + 
  coord_flip()

for (i in c(4:ncol(plot))){
  if (!(names(plot)[i] %in% BCP_all)) {
    p <- p + 
      eval(parse(text = paste("geom_point( aes(x=group, y= plot[,",i,"]),shape = 21,color='black',fill='#C58CA9', size=4)"))) #BV
  }
  if (names(plot)[i] %in% BCP_all){
    p <- p + 
...
IF = c(which(table_IDP_BCP_ND_PD$Total_IDP>0 & table_IDP_BCP_ND_PD$Total_ND > 0), 
  which(table_IDP_BCP_ND_PD$Total_IDP>0 & table_IDP_BCP_ND_PD$Total_BCP > 0),
  which(table_IDP_BCP_ND_PD$Total_IDP>0 & table_IDP_BCP_ND_PD$Total_PD > 0))
plot = table_IDP_BCP_ND_PD[IF, ]
plot = rbind(plot, Total = colSums(plot))   #行和
plot = plot[,-which(plot['Total',]==0)]
...
p <- ggplot(plot) +
  ...
  ylab("Traits") +
  scale_y_continuous(limits=c(1,ncol(plot)-3), breaks=seq(1,ncol(plot)-3,1),labels = c(names(plot)[4:ncol(plot)])) +
  theme(axis.text.x=element_text(angle=45, size=12, hjust = 1,color = 'black',face="bold"),
        axis.text.y=element_text(size=12, color = 'black',face="bold"), 
        axis.title.x = element_text(size=15, color = 'black',face="bold"),
        axis.title.y = element_text(size=15, color = 'black',face="bold"),
        plot.title = element_text(size=16, color = 'black',face="bold")) +
  ggtitle("Shared cell-specific genes between brain structure and at least two categories of brain disorders") + 
  coord_flip()

for (i in c(4:ncol(plot))){
  if (names(plot)[i] %in% IDP_all) {
    p <- p + 
      eval(parse(text = paste("geom_point( aes(x=group, y= plot[,",i,"]),shape = 21,color='black',fill='#ff9900', size=4)"))) 
  }
  if (names(plot)[i] %in% ND_all){
    p <- p + 
      eval(parse(text = paste("geom_point( aes(x=group, y= plot[,",i,"]),shape = 21,color='black',fill='#146eb4', size=4)")))
  } 
  if (names(plot)[i] %in% BCP_all){
    p <- p + 
      eval(parse(text = paste("geom_point( aes(x=group, y= plot[,",i,"]),shape = 21,color='black',fill='#CE77C4', size=4)")))
  } 
  if (names(plot)[i] %in% PD_all){
    p <- p + 
      eval(parse(text = paste("geom_point( aes(x=group, y= plot[,",i,"]),shape = 21,color='black',fill='#009526', size=4)")))
  } 
}
p
